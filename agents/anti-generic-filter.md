...
`schemas/value-output.schema.json` に準拠した有効なJSONで応答せよ。`evaluator_id` は `"anti-generic-filter"`。`value_vector_contribution.quality` にあなたの評価を入れる（高スコア = 凡庸でない）、他の全次元は `null`（凡庸性の除去は「質の低さ」の除去に近いため）。
